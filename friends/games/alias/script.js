...
let gameState = {
    activeScreen: 'main',
    gameType: 'show', // 'show' или 'explain'
    selectedCategories: ['objects', 'animals', 'actions'],
    gameMode: 'classic', // 'classic' или 'teams'
    wordsCount: 10,
    timerSeconds: 60,
    isPlaying: false,
    currentWordIndex: 0,
...
function updateGameTypeUI() {
    ...
    if (gameState.gameType === 'show') {
        gameIcon.className = 'fas fa-hand-sparkles';
        gameTitle.textContent = 'КРОКОДИЛ';
        gameSubtitle.textContent = 'Объясняй слова без слов';
        typeHint.textContent = 'Показывай слова жестами и мимикой';
    } else {
        gameIcon.className = 'fas fa-comments';
        gameTitle.textContent = 'АЛИАС';
        gameSubtitle.textContent = 'Объясняй слова словами';
        typeHint.textContent = 'Объясняй слова другими словами';
    }
}
...
function startGame() {
    ...
    gameState.selectedCategories.forEach(category => {
        if (gameWords[category]) {
            gameState.wordsList = gameState.wordsList.concat(gameWords[category]);
        }
    });
    ...
    if (gameState.wordsList.length === 0) {
        showAlert('Нет слов по выбранным настройкам! Выберите другие категории.');
        return;
    }
...
function updateGameUIForType() {
    const gameTypeIndicator = document.getElementById('gameTypeIndicator');
    const rulesText = document.getElementById('rulesText');
    const gameTypeResult = document.getElementById('gameTypeResult');
    
    if (gameState.gameType === 'show') {
        gameTypeIndicator.innerHTML = '<i class="fas fa-hand-sparkles"></i> Показывать';
        rulesText.textContent = 'Показывай слова жестами и мимикой, без слов';
        gameTypeResult.innerHTML = '<i class="fas fa-hand-sparkles"></i>';
    } else {
        gameTypeIndicator.innerHTML = '<i class="fas fa-comments"></i> Объяснять';
        rulesText.textContent = 'Объясняй слова другими словами, без жестов';
        gameTypeResult.innerHTML = '<i class="fas fa-comments"></i>';
    }
}
...
function correctGuess() {
    const points = gameState.gameType === 'show' ? 3 : 1;
...
function loadSettings() {
    ...
    if (saved) {
        try {
            ...
            gameState.gameType = settings.gameType || 'show';
            ...
            // Обновление UI
            document.querySelectorAll('.type-btn').forEach(btn => {
                btn.classList.toggle('active', btn.dataset.type === gameState.gameType);
            });
