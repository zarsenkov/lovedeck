...
function updateStats() {
    ...
    const stats = JSON.parse(localStorage.getItem('lc_stats') || '{"completed": 0, "sessions": 0, "romance": 0, "fun": 0, "adult": 0, "dates": 0}');
    const favsCount = JSON.parse(localStorage.getItem('lc_favs') || '[]').length;
    const customsCount = JSON.parse(localStorage.getItem('lc_customs') || '[]').length;

    // 2. Вспомогательная функция для безопасного обновления текста
    ...
    // 3. Обновляем все поля. Если какого-то ID нет в HTML, ошибки не будет.
    safeSetText('stat-completed', stats.completed || 0);
    safeSetText('stat-sessions', stats.sessions || 0);
    safeSetText('stat-favorites', favsCount);
    safeSetText('stat-custom', customsCount);
    
    safeSetText('stat-romance', stats.romance || 0);
    safeSetText('stat-fun', stats.fun || 0);
    safeSetText('stat-adult', stats.adult || 0);
    safeSetText('stat-dates', stats.dates || 0);
}
